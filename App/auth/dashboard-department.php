...
<body class="admin-page">
                        ...
                        <?php if (!in_array($post['status'], ['resolved', 'rejected'], true)): ?>
                            <div class="status-form">
                                ...
                                <select id="department-status-<?= (int) $post['id'] ?>" class="status-select"
                                    ...
                                    <option value="in_process" <?= $post['status'] === 'in_process' ? 'selected' : '' ?>>Diproses
                                    </option>
                                    <option value="communicated" <?= $post['status'] === 'communicated' ? 'selected' : '' ?>>
                                        Dikomunikasikan</option>
                                    <option value="resolved" <?= $post['status'] === 'resolved' ? 'selected' : '' ?>>Selesai
                                    </option>
                                </select>
                        ...
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;flex-wrap:wrap;">
                            <div style="flex:1;min-width:0;">
                                <div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.4rem;flex-wrap:wrap;">
                                    <span
                                        style="font-size:.7rem;font-weight:700;text-transform:uppercase;color:#008891;"><?= e($post['category'] ?? 'Lainnya') ?></span>
                                    <span
                                        style="display:inline-block;padding:.2rem .65rem;border-radius:999px;font-size:.68rem;font-weight:700;color:white;background:<?= $sl['color'] ?>;">
                                        <?= e($sl['label']) ?>
                                    </span>
                                    ...
                                    <span style="font-size:.7rem;color:#9ca3af;">•</span>
                                    <span
                                        style="font-size:.7rem;color:#9ca3af;"><?= date('d M Y', strtotime($post['created_at'])) ?></span>
                                </div>
                                <h3 style="margin:0 0 .5rem;font-size:1rem;font-weight:700;color:#111827;">
                                    <?= e($post['title']) ?></h3>
                                <div style="display:flex;align-items:center;gap:.65rem;flex-wrap:wrap;">
                                    ...
                                    <span style="font-size:.8rem;color:#6b7280;"><?= (int) $post['comments_count'] ?>
                                        komentar</span>
                                ...
                                <?php if ($descriptionExcerpt !== ''): ?>
                                    <p
                                        style="max-width:720px;margin:.75rem 0 0;padding:.7rem .85rem;border-left:3px solid #008891;border-radius:.25rem;background:#f8fafc;color:#6b7280;font-size:.84rem;line-height:1.55;">
    ...
    <div class="admin-modal" id="departmentPostModal" hidden>
        <div class="admin-modal-panel" role="dialog" aria-modal="true" aria-labelledby="departmentModalTitle">
            <button class="modal-close" id="departmentModalClose" type="button"
    ...
    <!-- Toast -->
    <div id="deptToast"
        style="position:fixed;bottom:1.5rem;right:1.5rem;padding:.85rem 1.25rem;border-radius:.6rem;font-size:.9rem;font-weight:600;color:white;z-index:9999;display:none;box-shadow:0 8px 24px rgba(0,0,0,.2);">
    </div>
